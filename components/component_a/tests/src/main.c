 ...
 *
 * ztest Unit-Tests für component_a
 *
...
static struct component_a_dev test_dev;
...
ZTEST_SUITE(component_a, NULL, suite_setup, before_each, NULL, NULL);
 ...
 */
ZTEST(component_a, test_init_valid)
{
    int ret = component_a_init(&test_dev);
 ...
 */
ZTEST(component_a, test_init_null_pointer)
{
    int ret = component_a_init(NULL);
 ...
 *
 * ZTEST(component_a, test_mein_test)
 * {
