...
sudo apt-get install -y git cmake ninja-build gperf ccache dfu-util device-tree-compiler \
  ...
  make gcc gcc-multilib g++-multilib libsdl2-dev libmagic1 libhidapi-hidraw0 clang-format
```
...
Für Schüler gibt es zusätzlich ein vereinfachtes Helferskript:

```bash
cd ~/zephyrproject/zephyr-pico-w-starter
./scripts/easy.sh build
```

Weitere einfache Befehle:

```bash
./scripts/easy.sh rebuild   # kompletter Neuaufbau
./scripts/easy.sh test      # alle Tests (Twister)
./scripts/easy.sh flash     # per west flash
./scripts/easy.sh run       # per picotool
```
